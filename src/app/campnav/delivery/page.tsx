"use client"

import { useState } from "react"
import Link from "next/link"
import { ArrowLeft, Truck } from "lucide-react"
import { useMutation } from "convex/react"
import { useRouter } from "next/navigation"

import { Card } from "@/components/ui/card"
import { Input } from "@/components/ui/input"
import { useAuth } from "@/components/auth-provider"
import { api } from "../../../../convex/_generated/api"

export default function DeliveryPage() {
    const router = useRouter()
    const { user } = useAuth()
    const [items, setItems] = useState("")
    const [from, setFrom] = useState("")
    const [toRoom, setToRoom] = useState("")
    const [instructions, setInstructions] = useState("")
    const [isSubmitting, setIsSubmitting] = useState(false)

    const createRequest = useMutation(api.requests.create)

    async function handleSubmit() {
        if (!user || !toRoom.trim() || !items.trim()) return
        setIsSubmitting(true)

        try {
            await createRequest({
                userId: user._id,
                type: "delivery",
                roomNumber: toRoom.trim(),
                description: `Deliver: ${items.trim()}${from ? ` from ${from.trim()}` : ""}. Instructions: ${instructions}`,
                priority: "important",
            })
            alert("Delivery request submitted!")
            router.push("/campnav/requests")
        } catch (err) {
            console.error("Failed to submit delivery request:", err)
            alert("Error submitting request. Please try again.")
        } finally {
            setIsSubmitting(false)
        }
    }

    return (
        <div className="space-y-6 pb-4">
            <header className="space-y-2">
                <div className="flex items-center gap-2 text-xs text-muted-foreground">
                    <Link href="/campnav/services" className="inline-flex items-center gap-1">
                        <ArrowLeft className="h-3 w-3" />
                        <span>Services</span>
                    </Link>
                    <span>/</span>
                    <span>Delivery</span>
                </div>
                <h1 className="text-xl font-semibold">Delivery Service</h1>
                <p className="text-xs text-muted-foreground">
                    Request item transport or courier services within the camp.
                </p>
            </header>

            <Card className="space-y-4 rounded-2xl border bg-card/90 p-4 shadow-sm">
                <div className="space-y-1 text-xs">
                    <label className="block text-[11px] font-medium text-muted-foreground">
                        What needs to be delivered?
                    </label>
                    <Input
                        placeholder="e.g. luggage, package, gear..."
                        className="h-10 rounded-xl"
                        value={items}
                        onChange={(e) => setItems(e.target.value)}
                    />
                </div>

                <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-1 text-xs">
                        <label className="block text-[11px] font-medium text-muted-foreground">
                            Pickup From (Optional)
                        </label>
                        <Input
                            placeholder="e.g. Main Gate"
                            className="h-10 rounded-xl text-[11px]"
                            value={from}
                            onChange={(e) => setFrom(e.target.value)}
                        />
                    </div>
                    <div className="space-y-1 text-xs">
                        <label className="block text-[11px] font-medium text-muted-foreground">
                            Deliver To (Room #)
                        </label>
                        <Input
                            placeholder="e.g. cabin 101"
                            className="h-10 rounded-xl text-[11px]"
                            value={toRoom}
                            onChange={(e) => setToRoom(e.target.value)}
                        />
                    </div>
                </div>

                <div className="space-y-1 text-xs">
                    <p className="text-[11px] font-medium text-muted-foreground">Special Instructions</p>
                    <textarea
                        rows={4}
                        className="min-h-[96px] w-full rounded-xl border border-input bg-background px-3 py-2 text-xs outline-none ring-ring/50 focus-visible:border-ring focus-visible:ring-[3px]"
                        placeholder="Any specific delivery instructions..."
                        value={instructions}
                        onChange={(e) => setInstructions(e.target.value)}
                    />
                </div>

                {/* Actions */}
                <div className="mt-4 flex gap-3 text-sm">
                    <button
                        type="button"
                        onClick={() => router.back()}
                        className="flex-1 rounded-full border bg-background py-2 text-[12px] font-semibold text-muted-foreground hover:bg-muted transition-colors"
                    >
                        Cancel
                    </button>
                    <button
                        type="button"
                        disabled={isSubmitting || !user || !toRoom.trim() || !items.trim()}
                        onClick={handleSubmit}
                        className="flex-1 rounded-full bg-primary py-2 text-[12px] font-semibold text-primary-foreground shadow-lg shadow-primary/20 disabled:opacity-50 transition-all transform active:scale-95"
                    >
                        {isSubmitting ? "Submitting..." : "Request Delivery"}
                    </button>
                </div>
            </Card>

            {/* Info Card */}
            <Card className="rounded-2xl border bg-primary/5 p-4">
                <div className="flex gap-3">
                    <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-primary/10 text-primary">
                        <Truck className="h-5 w-5" />
                    </div>
                    <div className="space-y-1">
                        <p className="text-xs font-semibold">Fast Internal Transport</p>
                        <p className="text-[10px] leading-relaxed text-muted-foreground">
                            Our team provides quick delivery services for your luggage, laundry, or any other items between camp locations.
                        </p>
                    </div>
                </div>
            </Card>
        </div>
    )
}
